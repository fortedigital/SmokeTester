...
namespace RobotsTxt
{
    ...
    public class Robots : IRobotsParser
    {
        ...
        public bool IsPathAllowed(string userAgent, string path)
        {
            ...
            var rulesForThisRobot = specificAccessRules.FindAll(x => userAgent.IndexOf(x.For, StringComparison.OrdinalIgnoreCase) >= 0);
        ...
        public long CrawlDelay(string userAgent)
        {
            ...
            var rulesForThisRobot = crawlDelayRules.FindAll(x => x.For.IndexOf(userAgent, StringComparison.OrdinalIgnoreCase) >= 0);
